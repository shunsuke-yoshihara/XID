...
for {set n $START_PATTERN} {$n >= 1} {incr n -1} {
  ...
  for {set bit 0} {$bit < $SI_BIT} {incr bit} {
    ...
    # python処理1: b20_ET1.stil を読み込み、$n パタン目のtest_si の$bit 目を"N(don't care)"に変更したパターンを1パターンSTIL出力し、そのパスを返す(b20_ET1.stil は変更しない)
    ...
    if {[file size $fault_file] == 0} {
      ...
      # python処理2: b20_ET1.stil を読み込み、$n パターン目のtest_siの$bit 目を探し、"N"(don't care)に置換する
...
################################################################################################################################################
  ...
  for {set bit 0} {$bit < $PI_BIT} {incr bit} {
    ...
    # python処理1: b20_ET1.stil を読み込み、$n パタン目のpi の$bit 目を"N(don't care)"に変更したパターンを1パターンSTIL出力し、そのパスを返す(b20_ET1.stil は変更しない)
    set stil_file [timed_eval make_candidate_python {
    ...
    if {[file size $fault_file] == 0} {
      ...
      # python処理2: b20_ET1.stil を読み込み、$n パターン目のpiの$bit 目を探し、"N"(don't care)に置換する
      timed_eval apply_x_python {
