...
% cd python
% python3 Basic_CB_Partition_to_STIL.py
```

2. 分割後のSTILを使用して、1パターンごとのテストベンチ作成

```text
# 出力先は、${Circuit}/${Partitioning_Stil_Testbench}
% cd b14
% mkdir -p Partitioning_Stil_Testbench
% set pattern = 0
# 663 には、STILファイル数+1 を使用(0から番号付けされているため)
% while ( $pattern <= 663 )
while? stil2verilog Partitioning_Stil/b14_pn${pattern}.stil Partitioning_Stil_Testbench/b14_pn${pattern}_tb
while? @ pattern++
while? end
```
...
% dc_shell
dc_shell> source dc_netlist_parser/tcl/src/write_signal_list.tcl
...
```text
% cd b14
% mkdir -p Partitioning_Stil_Testbench_VCD
% set pattern = 0
...
while? vlogan -full64 Partitioning_Stil_Testbench/b14_pn${pattern}_tb.v _b14.v b14_dump.v -v library ~ohtake/cad/lib/Synthesis/nangate45nm.v
...
while? ./simv +VCD=Partitioning_Stil_Testbench_VCD/b14_pn${pattern}.vcd
